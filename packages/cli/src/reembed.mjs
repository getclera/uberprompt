...
async function reembedPrompt(db, env, doc, dryRun) {
  const fragIdx = [];
  const texts = [];
  ...
  const hasDesc = Array.isArray(doc.descriptionEmbedding) && doc.description;
  ...
  const set = { updatedAt: new Date(), updatedBy: "cli" };
  fragIdx.forEach((idx, j) => {
    set[`fragments.${idx}.embedding`] = vectors[j];
  });
  if (hasDesc) set.descriptionEmbedding = vectors[vectors.length - 1];
  const res = await db.collection("prompts").updateOne({ _id: doc._id, version: doc.version }, { $set: set });
...
export async function runReembed(repoRoot, opts = {}) {
  ...
  try {
    ...
    for (const lesson of lessons) {
      ...
      await db.collection("lessons").updateOne({ _id: lesson._id }, { $set: { embedding: vector } });
